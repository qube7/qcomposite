...
{
    /// <summary>
    /// Provides extension methods for the <see cref="DispatcherObject"/>/<see cref="IDispatcherObject"/>.
    /// </summary>
    public static class DispatcherObjectExtensions
    {
        ...
        /// <summary>
        /// Performs the specified <see cref="Action{T}"/> on the <see cref="DispatcherObject"/> on the calling thread if it has access to the <see cref="DispatcherObject"/> or synchronously on the thread the <see cref="DispatcherObject"/> is associated with.
        /// </summary>
        ...
        public static void Update<T>(this T obj, Action<T> callback) where T : DispatcherObject
        {
            Requires.NotNull(obj, nameof(obj));
        ...
        /// <summary>
        /// Performs the specified <see cref="Action{T}"/> on the <see cref="DispatcherObject"/> on the calling thread if it has access to the <see cref="DispatcherObject"/> or asynchronously on the thread the <see cref="DispatcherObject"/> is associated with.
        /// </summary>
        /// <typeparam name="T">The type of the source dispatcher object.</typeparam>
        /// <param name="obj">The source dispatcher object.</param>
        /// <param name="callback">A delegate to perform on <paramref name="obj"/>.</param>
        /// <returns>An object that can be used to interact with the delegate as it is pending execution in the event queue.</returns>
        public static DispatcherOperation UpdateAsync<T>(this T obj, Action<T> callback) where T : DispatcherObject
        {
            Requires.NotNull(obj, nameof(obj));
            Requires.NotNull(callback, nameof(callback));
