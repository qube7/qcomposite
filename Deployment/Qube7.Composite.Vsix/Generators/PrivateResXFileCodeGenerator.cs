    ...
    public class PrivateResXFileCodeGenerator : BaseCodeGenerator
    {
        ...
        private string GenerateCode(Dictionary<string, ResXData> properties, string resxFile, string defaultNamespace)
        {
            ...
            if (properties.Count > 0)
            {
                ...
                NameSyntax name = SyntaxFactory.ParseName(defaultNamespace);

                unit = unit.AddMembers(
                    SyntaxFactory.NamespaceDeclaration(name).AddMembers(
                                        ...
                                        SyntaxFactory.VariableDeclarator(FieldName).WithInitializer(
                                            SyntaxFactory.EqualsValueClause(
                                                SyntaxFactory.ObjectCreationExpression(SyntaxFactory.ParseName(typeof(ResourceManager).FullName)).AddArgumentListArguments(
                                                    SyntaxFactory.Argument(SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal(SyntaxFactory.QualifiedName(name, SyntaxFactory.IdentifierName(fileName)).ToString()))),
        ...
        {
            TypeSyntax typeName = SyntaxFactory.ParseTypeName(Culture);

            IdentifierNameSyntax propertyName = SyntaxFactory.IdentifierName(Current);
                    ...
                    SyntaxFactory.AccessorDeclaration(SyntaxKind.GetAccessorDeclaration).AddBodyStatements(
                        SyntaxFactory.ReturnStatement(
                            SyntaxFactory.InvocationExpression(
                                ...
                                SyntaxFactory.Argument(SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal(data.Name))),
                                SyntaxFactory.Argument(SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, typeName, propertyName))
                                )
                            )
                        )
